...
class Visualizer:
    ...
    def __init__(self, voronoi: Polygon, canvas_offset=1, figsize=(8, 8)):
        self.voronoi = voronoi
        self.min_x, self.max_x, self.min_y, self.max_y = self._canvas_size(voronoi.bounds, canvas_offset)
    ...
    def show(self, block=True, **kwargs):
        ...
        # self.canvas.spines['left'].set_position('zero')  # y 轴穿过 x = 0
        # self.canvas.spines['bottom'].set_position('zero')  # x 轴穿过 y = 0
    ...
    def plot_all(self, polygon=False, edges=True, vertices=True, sites=False,
    ...
    def plot_polygon(self, poly, **kwargs):
        ...
        self.canvas.add_collection(collection, autolim=True)
        # self.canvas.autoscale_view()
        return self

    def plot_bisector(self, bisector, t=100, rotate=True, **kwargs):
        """绘制bisector"""
        if bisector is None:
            return self

        if isinstance(bisector, Ray):
            O = bisector.start_point
            y = O + t * bisector.direction
            self.canvas.plot([O[0], y[0]], [O[1], y[1]], zorder=20, color=Colors.EDGE, **kwargs)
        elif isinstance(bisector, Line):
            O = bisector.start_point
            y1 = O + t[0] * bisector.direction
            y2 = O + t[1] * bisector.direction
            self.canvas.plot([y1[0], y2[0]], [y1[1], y2[1]], zorder=20, color=Colors.EDGE, **kwargs)
        elif isinstance(bisector, Parabola):
            func = bisector.func
            if rotate:
                rotate_matrix = bisector.rotate_matrix
                min_x = np.dot(rotate_matrix, np.array([self.min_x, 0]))
                max_x = np.dot(rotate_matrix, np.array([self.max_x, 0]))
                x = linspace(min_x[0], max_x[0], 100)
            else:
                x = linspace(self.min_x, self.max_x, 100)

            y = func(x)
            self.canvas.plot(y[:, 0], y[:, 1], zorder=20, color=Colors.EDGE, **kwargs)
        elif isinstance(bisector, LineString):
            x, y = bisector.xy
            self.canvas.plot(x, y, zorder=20, color=Colors.EDGE, **kwargs)

        return self

    def plot_vertices(self, vertices=None, label=True, **kwargs):
        """
        Display the intersections of the edges.
        ...
        label: annotate vertices or not
        ...
        """
        if vertices is None:
            shell = self.voronoi.exterior.coords[:]
            holes = [hole.coords[:] for hole in self.voronoi.interiors]

            vertices = shell + holes

            vertices = vertices or self.voronoi.coords

        xs = [vertex[0] for vertex in vertices]
        ys = [vertex[1] for vertex in vertices]

        # Scatter points
        self.canvas.scatter(xs, ys, s=50, color=Colors.VERTICES, zorder=30, **kwargs)

        # props = dict(color=Colors.TEXT, **kwargs)
        if label:
            for ind, vertex in enumerate(vertices[:-1]):
                self.canvas.text(vertex[0], vertex[1], 'v{}'.format(ind), zorder=11)
    ...
    def plot_intersection(self, points=None, color=Colors.CELL_POINTS, zorder=10, **keywords):
        """
        Display the cell points (a.k.a. sites).
        ...
        # Scatter points
        self.canvas.scatter(xs, ys, color=color, zorder=zorder, **keywords)
    ...
    def plot_edges(self, edges=None, draw_boundary=False, show_labels=True, color=Colors.EDGE, **kwargs):
        """
        Display the borders of the cells.
        ...
        for key, edge in edges.items():
            start, end = edge.origin, edge.end

            x, y = edge.geom.xy

            if show_labels:
                self.canvas.text(start.xd, start.yd, s=f"{start}", size=12, color=Colors.TEXT, zorder=15)
                self.canvas.text(end.xd, end.yd, s=f"{end}", size=12, color=Colors.TEXT, zorder=15)

            if not draw_boundary:
                if edge.twin is not None:
                    self.canvas.plot(x, y, color)
                    # print("{}:{},{} {}:{},{}".format(start, x[0], y[0], end, x[1], y[1]))
            else:
    ...
    @staticmethod
    def _canvas_size(bounding_polygon: Polygon.bounds, offset):
        max_y = bounding_polygon[3] + offset
        max_x = bounding_polygon[2] + offset
        min_x = bounding_polygon[0] - offset
        min_y = bounding_polygon[1] - offset
