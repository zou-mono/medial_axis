...
class HalfEdge:
    def __init__(self, incident_segment, twin=None, origin=None):
        """
        ...
        incident_segment: Segment
            The segment of which this edge is the border
        twin: HalfEdge
        ...
        self.incident_segment = incident_segment
    ...
    def __repr__(self):
        if self.incident_segment is not None:
            return f"{self.incident_segment}/{self.twin.incident_segment or '-'}"
    ...
    def delete(self):
        ...
        if self.incident_segment is not None and self.incident_segment.first_edge == self:

            # Incident points should remain the same
            assert (
                    self.next is None or self.next.incident_point == self.incident_segment
            ), f"Incident segment {self.next.incident_point} and {self.incident_segment} do not match"

            # Set the new "first edge" pointer
            self.incident_segment.first_edge = self.next